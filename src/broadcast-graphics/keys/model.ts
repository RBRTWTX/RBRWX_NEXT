...
export interface WeatherKey { gradient?: string; ticks?: { label: string; position: number }[]; id: string; name: string; unit: string; source: string; bins: WeatherKeyBin[] }
...
export function resolveWeatherKey(selection: KeySelection, weatherKeyId?: string, extraKeys: readonly WeatherKey[] = []): WeatherKey | undefined {
  const id = selection === 'auto' ? weatherKeyId : selection;
  return [...weatherKeys, ...extraKeys].find(key => key.id === id);
}
